<template>
    <div :class="[{ 'fullscreen': s_fullScreen, 'shadow': boxShadow }]" class="v-note-wrapper markdown-body" :style="{'box-shadow': boxShadow ? boxShadowStyle : ''}">
       <!-- 提示框 -->
       <span id="ruler"></span>
       <div id="tipPop" class="hidden"></div>
       <!--工具栏-->
...
export default {
    ...
    created() {
        ...
        String.prototype.visualLength = function(size,family)
        {
            ...
            ruler.style ='font-size:' +  (size || 'inherit') +';font-family:' + (family || 'inherit') + ';'
    ...
    methods: {
        showTip(text = "",e) {
            // 鼠标跟随tip
            let textWidth = text.visualLength("16px")
            let sgTip = document.querySelector("#tipPop")
            sgTip.innerHTML = text
            sgTip.className = 'show'
            let x = window.innerWidth - textWidth - e.target.parentNode.offsetWidth - 20,
                y = e.target.offsetTop - (document.documentElement.scrollTop  | document.body.scrollTop) + 20
            sgTip.style.position = 'fixed'
            sgTip.style.zIndex = 999999
            sgTip.style.fontSize = '16px'
            if(textWidth > 150){
                sgTip.style.width = '150px'
                sgTip.style.left = (x + textWidth - 150) + "px"
            }else{
                sgTip.style.width = ''
                sgTip.style.left = x + "px"
            }
            sgTip.style.top = y + "px"
            sgTip.style.background = '#F8C301'
            sgTip.style.borderRadius = '5px'
            sgTip.style.padding = '0 5px 0 5px'
            sgTip.style.lineHeight = '20px'
            this.tipContent = text
        },
        hideTip() {
            let sgTip = document.querySelector("#tipPop")
            sgTip.innerHTML = ''
            sgTip.className = 'hidden'
        },
